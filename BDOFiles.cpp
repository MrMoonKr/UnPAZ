...
/// ***** FileEntry Structure ***** ///
FileEntry::FileEntry() : uiFileHash(0),
                         uiFolderNum(0),
                         uiFileNum(0),
                         uiPazNum(0),
                         uiOffset(0),
...
uint32_t BDOFile::ExtractFileMask(std::string sFileMask, fs::path OutputPath)
{
    ...
    if (sFileMask.empty())
    { /// turn off progressbar when listing all files
        this->SetQuiet(true);
    }
    ...
    {
        if (bProgress)
        {                    /// delete current line (progress bar)
            printProgress(); /// delete current line (progress bar)
        }
...
void BDOFile::internalExtractFile(fs::path FilePath, fs::path PazName, uint32_t uiOffset, uint32_t uiCompressedSize, uint32_t uiOriginalSize)
{
    ...
    if (FilePath.has_parent_path() && !fs::exists(FilePath.parent_path()))
    {
        ...
        {
            switch (createPathPrompt(FilePath))
            { ///(Y)es / (a)lways / (s)kip file / (e)xit
            case 'e':
                exit(0);
            case 'a':
    ...
    else
    {
        ...
        if (!this->bOverwriteFiles)
        {
            if (fs::exists(FilePath))
            {
                ...
                else
                {
                    ...
                    { /// options: (o)verwrite / (R)ename / overwrite (a)ll / re(n)ame all / (e)xit
                    case 'e':
                        exit(0);
                    case 'a':
        ...
        if (!this->GetMobile())
        {
            ...
            if ((decrypted[0] == 0x6F || decrypted[0] == 0x6E) && uiCompressedSize > 9)
            {
                ...
                if (uiSize == uiOriginalSize)
                { /// We can consider data header as valid. Size in data header is the same as size in .meta/.paz file.
...
MetaFile::MetaFile( std::filesystem::path FileName, bool bQuiet ) 
    : uiClientVersion(0)
    , uiFilesCount(0)
    , uiFileNamesCount(0)
    , uiFoldersCount(0)
...
/**
 * Read meta file and store data to vFilesTable
 * @param metaFileName path to .meta file
 */
void MetaFile::ReadSource( std::filesystem::path metaFileName )
{
    ...
    // open file stream
    metaFile.open( metaFileName.string(), std::ios::binary );
    if ( !metaFile.is_open() )
    {
        this->exitError( -2, metaFileName.string() );
    }

    this->SetArchivePath( metaFileName.parent_path() );

    metaFile.read( reinterpret_cast<char *>( readBuffer ), 8 );
    ...
    metaFile.seekg( uiPazCount * 12, std::ios::cur );

    metaFile.read( reinterpret_cast<char*>(readBuffer), 4 );
    ...
    metaFile.read( reinterpret_cast<char*>( pFilesInfo ), this->uiFilesCount * 28 );
    ...
    if ( pFolderNamesEncrypted == 0 )
    {
        this->exitError(-3);
    }
    pFolderNamesDecrypted = new uint8_t[uiFolderNamesLength];
    if ( pFolderNamesDecrypted == 0 )
    {
        this->exitError(-3);
    }

    metaFile.read( reinterpret_cast<char*>( pFolderNamesEncrypted ), uiFolderNamesLength );
    ...
    if (!this->GetMobile())
    {
        ...
        if ( pFileNamesEncrypted == 0 )
        {
            this->exitError(-3);
        }

        metaFile.read( reinterpret_cast<char*>( pFileNamesEncrypted ), uiFileNamesLength );
        this->ICEdecrypt( pFileNamesEncrypted, pFileNamesDecrypted, uiFileNamesLength );

        delete[] pFileNamesEncrypted;
    }
    else
    {
        metaFile.read(reinterpret_cast<char*>( pFileNamesDecrypted ), uiFileNamesLength );
    }
...
PazFile::PazFile( std::filesystem::path FileName, bool bQuiet) 
    : uiPazHash(0)
    , uiFilesCount(0)
{
    this->SetQuiet( bQuiet );
...
void PazFile::ReadSource( std::filesystem::path FileName )
{
    ...
    if (!ifsPazFile.is_open())
        this->exitError(-2, FileName.string());
