...
namespace BDO
{
    ...
    uint32_t blackdesert_unpack_core(uint8_t *pInput, uint8_t *pOutput, uint32_t uiDecompressedLength, uint8_t *pOutput2, uint32_t pInput_size)
    {
        ...
        if (pInput[0] & 0x02)
        { /// if bit 1 is set, length is stored as long (4 bytes):  ???? ??1?
        ...
        { /// if bit 1 is not set, length is stored as byte:                         ???? ??0?
        ...
        while (1)
        {
            while (1)
            {
                ...
                {                                                     /// if bit 0 is not set and bit 1 is set                           ???? ???? ???? ???? ???? ???? ???? ??10
                ...
                {                                                 /// if bit 0 is set and bit 1 is not set                           ???? ???? ???? ???? ???? ???? ???? ??01
                ...
                {                                                /// if bits 0 and 1 are not set                                                                        ???? ???? ???? ???? ???? ???? ???? ??00
                ...
                { /// interesting is that this will read 4 bytes, but pointer is incremented by 3. It means that 4th byte will be always overwritten by the next cycle.
    ...
    uint32_t decompress(uint8_t *pInput, uint8_t *pOutput)
    {
        ...
        if (pInput[0] & 0x02)
        { /// if data header has set bit 1, length in header is stored as long (4 bytes) - ???? ??1?
            length = (*(uint32_t *)(pInput + 5));
        }
        else
        { /// if data header has not set bit 1, length in header is stored as byte - ???? ??0?
            length = pInput[2];
        }

        if (pInput[0] & 0x01)
        { /// if data header has set bit 0, data are compressed and will be sent to internal decompression function - ???? ???1
            length = blackdesert_unpack_core(pInput, pOutput, length, pOutput, 0);
        }
        else
        { /// if data header has not set bit 0, data are not compressed. Skip header and copy data to pOutput. - ???? ???0
